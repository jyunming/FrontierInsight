...
- **Checks that aren't just another LLM** — most guardrails in tools like this end in a language model reading text, which cannot catch a number copied wrong. FI adds two that are arithmetic: a **numeric oracle** comparing the paper's figures against what the run actually computed (digit transposition and rounding-unexplainable near-misses are reported for a human to judge, not auto-enforced — a regex over prose will always have an opinion about a DOI, and a wrong rewrite costs more than a flagged number), and a **plausibility gate** enforcing the range and unit bounds the design declared — so a unit error, a sign flip, or a diverging value the script capped to look in range goes back for repair instead of into a paper. Each figure also carries a record of what it actually draws, so a caption describing a line that lies flat on its axis goes back to the reviewer.
