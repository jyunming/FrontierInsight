...
def test_no_template_narrates_the_engine() -> None:
    """A rendered paper must read as a standalone document, never as a
    report on the tool that produced it. Four venue templates hardcoded
    the byline ``\\author{Frontier Insight \\\\ Generated by an automated
    research pipeline}``, stamping that phrase into every PDF regardless
    of the write prompt's no-engine-narration rule. Guard against any
    template re-introducing engine/pipeline self-reference in static text."""
    banned = [
    ...
    for tpl in _all_templates():
        ...
        for phrase in banned:
            assert phrase not in low, (
                f"{tpl.parent.name}/template.tex contains engine "
                f"self-reference {phrase!r} — a paper must not name the "
                f"tool that produced it"
            )
