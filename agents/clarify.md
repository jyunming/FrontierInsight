...
```
{
  ...
  "topic_shape": {
    "question": "<one sentence asking which intellectual SHAPE this topic has — a specific testable comparison (experimental), a descriptive history / overview with no experiment and no data (survey), a broad synthesis of prior knowledge (review), one system / phenomenon to study in depth (case_study), or a position to argue (opinion)>",
    "default": "experimental" or "survey" or "review" or "case_study" or "opinion",
...
- The `topic_shape` slot orthogonally classifies the topic's INTELLECTUAL SHAPE — separate from `simulatability` (which asks "can Python answer this?"). Four shapes:
  ...
  * `survey` — the topic asks for a DESCRIPTIVE history, overview, or evolution of a subject, answerable purely by synthesising published sources with NO experiment AND NO dataset ("the evolution of X", "history of Y", "an overview of Z", "how W developed over time"). The whole paper is a narrative synthesis; there is nothing to measure and no data to collect. Trigger words: "history of", "evolution of", "overview of", "development of", "retrospective", "the story of", "over time". PREFER `survey` over `review` when the topic is a self-contained history/overview rather than a comparison or a knowledge-gap synthesis.
  * `review` — the topic asks for a synthesis of prior knowledge across a field, typically to compare approaches or map a landscape ("OPC importance in low-k1 era", "differences between A and B", "trends in resolution-enhancement techniques"). The right output is a literature synthesis that MAY be grounded by ONE narrow illustrative measurement, not a toy experiment masquerading as a benchmark. Trigger words: "importance", "differences", "review of", "landscape of", "trends in", "comparison of".
