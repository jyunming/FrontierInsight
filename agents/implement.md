...
- **Answer the design's oracles when asked.** The design's `protocol.oracles` name checks that do not rely on this script's own numbers being right (a closed form, a limiting case, an invariant, an exact small case, a second implementation). When the environment variable `FI_ORACLE` is `1`, do NOT run the sweep: run each declared oracle check on a small, fast case (seconds), print ONE line `ORACLE_JSON: {"checks": [{"name": "<the declared name>", "passed": true, "value": 0.98, "expected": 1.0, "tolerance": 0.05}, ...]}` and exit 0 if every check passed, 1 if any did not. Every declared oracle must appear by name. Never make a check pass by loosening its tolerance, skipping it or hard-coding its answer. When `FI_ORACLE` is not set, the script runs as usual and prints its `RESULT_JSON`.
