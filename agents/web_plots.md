...
**Plot only numbers a source actually states.** Every value you chart must be a quantity reported in the sources in the Inputs section — a percentage, count, year-value, sample size (N), effect size, growth/decline rate, or ranking — that you could literally point to in the source text. Mine the sources for every chartable angle the *reported data* supports (up to ~6 figures):
...
- Hard-code the data you extracted directly from the sources as Python literals at the top of the script. Do NOT read any files. Do NOT fabricate, interpolate, or "estimate" numbers — use ONLY values actually stated in the sources in the Inputs section. If a figure would need a number the sources don't give, leave that figure out.
...
- Does NOT set its own visual theme. A Frontier Insight house style (Palatino-style serif, a teal-anchored palette, a warm off-white background, a single baseline with a light horizontal grid, no tick marks) is applied automatically before your code runs. So: do NOT call `plt.style.use(...)`, do NOT hard-code hex colours for bars/lines (let them take the default colour cycle), and do NOT override the figure/axes background or re-add spines/ticks. If you must colour specific series distinctly, use the brand names already defined for you — `FI_TEAL, FI_GOLD, FI_CLAY, FI_SLATE, FI_SAGE, FI_SAND, FI_PLUM` — rather than arbitrary colours. (For the source stamp, `color="gray"` is fine.) Keep axis labels, ticks, and titles in plain ASCII — write `Earth radii` / `R_Earth` / `M_sun`, not exotic Unicode like `R⊕` or `M☉`, which the serif font may render as an empty box.
- Makes each figure read like a finished editorial graphic, not a raw plot:
  - Give the title a SUBSTANTIVE takeaway (`"Transit method dominates: 74% of 6,298 confirmed planets"`), not just a label — the title carries the message, so a separate subtitle is usually unnecessary (and prone to overlapping the title).
  - **Label values directly on the marks** — `ax.bar_label(bars, fmt="{:,.0f}")` or `ax.text(...)` — with thousands separators (`f"{v:,}"`), so the reader doesn't decode them off the axis. Then you can drop the y-axis label or keep it minimal.
  - Prefer **direct labels next to lines / wedges** over a separate legend; use a legend only when unavoidable.
  - For ranked categories with long names, use a **horizontal** bar chart (`ax.barh`) sorted by value — it reads far better than rotated x-labels.
  - Leave generous margins (the style sets `figsize` and tight bbox); don't cram 6 series into one axes — prefer a couple of focused figures.
