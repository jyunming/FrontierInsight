...
- Plan every figure as at most 3 panels, side by side in one row. A comparison that needs more panels (a grid over two factors, more than three strata) becomes SEVERAL figures of at most 3 panels each, one function and one file per figure, because a figure is shown at a fixed width and every extra panel shrinks the text of all of them.
