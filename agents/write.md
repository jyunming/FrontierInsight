...
6. **Dropping the uncertainty.** When the analysis reports a number with a confidence interval, effect size, or a multiple-comparison caveat, carry it into the paper — report the headline metric WITH its 95% CI ("RMSE 0.045, 95% CI 0.041–0.049") and with what the interval rests on, as the analysis gives it (a Wilson interval over N pooled trials, a bootstrap over N pooled observations, or a spread across N seeds, which is not the number of trials), cite the effect size when claiming one group beats another, and honour any "many comparisons" caveat rather than over-claiming a single difference. If the analysis says a result was single-seed (no replication), do not imply a precision the run doesn't have.
