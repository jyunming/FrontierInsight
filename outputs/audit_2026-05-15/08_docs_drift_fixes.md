...
**Missing from diagram**: `clarify` (Phase I, pre-#57), `execute_reflect` (Phase K),
`cross_check` (Phase L), `auto_collect_data` (#60), `wait_for_data` (#57), `data_load`
(#57), `ideate_reflect` (Phase M), `review_panel` fan-out + moderator (Phase N).
...
|---|---|---|---|---|---|
| 1 | `README.md` | Requirements (L37–41) — append after L41 | New optional-deps line: *"For `engine.no_simulation: true` topics that pull from web sources, optional `pip install httpx beautifulsoup4` enables the Wikipedia + WorldBank dataset adapters."* | S | S |
| 2 | `README.md` | "What you'll have after one quest" (L45–61) | Add to the file-tree block, under `.fi/run.log`: `data/auto_collected/*.md       ← if engine.no_simulation: true` (with one-line comment that it appears only on no-sim quests). | M | S |
...
| 6 | `docs/INSTALL.md` | After "System tools" table (L104–118) | New H3 **"Optional: dataset adapters"** — *"PR #61 + #62 add structured-data adapters that hit public APIs. Network reqs: HTTPS to `api.worldbank.org`, `en.wikipedia.org`. No additional `pip install` needed (uses `httpx` already in `requirements.txt`). Enable per-quest via `engine.dataset_adapters: [worldbank, wikipedia]`."* | M | S |
| 7 | `docs/INSTALL.md` | After Path 3 or in Troubleshooting (L153) | New troubleshooting bullet: **"`output.require_pdf: true` aborts pre-flight"** — list the install recipe for each missing tool (pandoc, pdflatex, tectonic) referenced by the new pre-flight error message in `core/preflight.py`. | M | S |
...
| 15 | `docs/USAGE.md` | YAML config schema (L131–208) — Provider block (L142–151) | Add the 3 missing `ProviderConfig` fields: `base_url: null` (comment: "for OpenAI-compatible custom endpoints"), `api_key_env: null` (comment: "override default env-var name"), `extra: {}` (comment: "transport-specific bag — e.g., proxy headers"). | M | S |
...
| 17 | `docs/USAGE.md` | After L171 (schema for `engine.dataset_adapters`) | Add a 3-line dedicated **Wikipedia adapter** sub-bullet mirroring the WorldBank one at L355–360. Cover: extracts entities from the query, fetches lead section + infobox, writes as Markdown with provenance + revision id. | M | S |
