...
| 17 | `docs/USAGE.md` | After L171 (schema for `engine.dataset_adapters`) | Add a 3-line dedicated **Wikipedia adapter** sub-bullet mirroring the WorldBank one at L355–360. Cover the ACTUAL behavior (per `core/datasets/wikipedia.py`): compresses query to top-6 keywords → hits `opensearch` for candidate titles → fetches `/page/summary/<title>` for each → writes Markdown with the page's `description` and `extract` in the BODY, and YAML front matter carrying `source: wikipedia`, `title`, canonical `url`, `wikipedia_type`, and `adapter: wikipedia`. (Do NOT describe infoboxes or revision-id storage — those are not in the implementation; also do not claim `description`/`extract` are in front matter — they live in the body.) | M | S |
