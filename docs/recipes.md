...
The agent reads them and cites them, every page: a scanned PDF (an older paper is usually page images) is read by OCR when RapidOCR or tesseract is installed (`pip install rapidocr onnxruntime`, part of `requirements.txt`), and the run log says which pages, if any, could not be read. RapidOCR fetches its models (about 32 MB, from modelscope.cn) the first time and runs offline afterwards; on a machine that is never online, copy `site-packages/rapidocr/models/` from one that has them, or use tesseract.
