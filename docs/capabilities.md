...
- **Reviewer panel** — N personas in parallel + moderator synthesis (`rigor_score` + `depth_score` axes). The built-in **methodologist** persona hard-flags four common-but-fatal patterns and forces the panel verdict to at most ``revise`` when any apply: circular evaluation (optimization objective shares model/simulator/distribution with the evaluation metric — "train-on-test"), single-point evaluation where a sweep is the field norm, admitted-weak baselines kept in the comparison instead of re-run, and metrics in dimensionless / grid-only units without a physical-grounding bridge. The persona must explicitly state non-applicability of each check rather than reach acceptance by silence.
