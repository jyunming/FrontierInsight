...
| Unified interactive interview across every frontend — `python launch.py --new` (CLI), `@fi /new` (VSCode chat), and `GET /interview` (`--serve` web UI) all drive the SAME 12-question Python interview defined in `core/interview.py` (with `core/interview_schema.json` as the JSON snapshot consumed by TS + HTMX). The question set covers topic / title / output bundle / paper format / research approach / **study depth** / **comparative baseline** (topic-tuned LLM-suggested default via `agents/clarify_preflight.md`) / **success metric** (topic-tuned) / **budget** (topic-tuned) / clarify mode / reviewer panel / Axon. CLI adds 2 questions for **provider** + **model** (curated per-provider list with an "Other (type your own)" escape hatch); VSCode skips those and pins `provider.model` from the active Copilot model via `vscode.lm.selectChatModels()`. Answers land in `engine.clarify_overrides` so the engine's clarify node short-circuits in auto mode (no wasted LLM call). | ✅ |
