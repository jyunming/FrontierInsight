...
| Pause-for-user-papers gate — set `knowledge.pause_for_user_papers: true` and the literature node pauses when any retrieved doc came back abstract-only (no full text available — typical for paywalled / Crossref / S2 metadata-only hits). Writes a `needs/<slug>.json` stub per missing paper with the citation metadata FI knows, creates `inputs/papers/README.md` with resume instructions, then exits rc=0 cleanly. The user downloads PDFs into `<quest_root>/inputs/papers/` and runs `fi --resume <quest_id>`; the literature node walks the new files (PDF via pypdf, MD/TXT verbatim), appends them as `source=user_supplied` literature entries, and continues. Pairs naturally with `knowledge.try_fetch_full_text`: turn that on first, then this gate only fires for the subset the host couldn't open-text-fetch. Default `false` — existing quests keep running unattended. | ✅ |
