...
| No-simulation / data-required mode — three-tier decision (logged at INFO as `[clarify] simulatability resolved: ... source=<yaml \| clarify_simulatability \| clarify_empirical_legacy \| default>`): (1) `engine.no_simulation: true` in YAML wins; (2) the clarify agent's new `simulatability` slot (`default: yes \| no \| uncertain` + `reason`) — `no` triggers; (3) legacy fallback on `empirical_vs_theoretical: empirical`. When triggered, the engine runs `clarify → ideate → literature → design`, writes `outputs/<id>/data/README.md` with instructions, and exits clean (rc=0). User drops empirical data into `data/` and re-runs `fi --resume <id>`. The `data_load` node walks the dir (reusing the summarizer's mixed-format walker), synthesizes a `result_json` from the contents, and the quest continues through `analyze → cross_check → write → review`. Useful for qualitative / social-science / archival topics where simulation would just invent data. | ✅ |
