...
- **Ensemble profile is a tier-1 quest decision** — `Multi-model ensemble` (`off` / `cross_check_only` / `ideate_and_check` / `full`) sits next to provider/model on `/interview` and the CLI's `--new` flow rather than under "Show advanced". `/tools/proposal` and `/tools/critique` carry the same picker; the form maps the picked profile through `core.interview.ensemble_model_trio(provider)` to a 3-model CSV and passes it via the existing `--proposal-ensemble` / `--critique-ensemble` CLI flags (merge defaults match each tool — `tournament` for proposal, `synthesize` for critique). The other 4 LLM tools (digest / portfolio / summarize / analyze) need core-side ensemble wiring before they can join.
