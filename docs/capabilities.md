...
| No-simulation / data-required mode — when `engine.no_simulation: true` in YAML (or clarify answers `empirical_vs_theoretical: empirical`), the engine runs `clarify → ideate → literature → design`, writes `outputs/<id>/data/README.md` with instructions, and exits clean (rc=0). User drops empirical data into `data/` and re-runs `fi --resume <id>`. The `data_load` node walks the dir (reusing the summarizer's mixed-format walker), synthesizes a `result_json` from the contents, and the quest continues through `analyze → cross_check → write → review`. Useful for qualitative / social-science / archival topics where simulation would just invent data. | ✅ |
