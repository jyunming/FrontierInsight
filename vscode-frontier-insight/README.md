...
A finished quest ends with `[FI] evidence: <level>` (`executed`, `internally_reconciled`, `protocol_runtime_matched`, `independently_validated`, `statistically_adequate` or `publication_ready`) and what stands between it and the next level; the record is `needs/EVIDENCE.json`. How the checks work is in [docs/rigor.md](../docs/rigor.md).
