...
Every quest writes `plan.md` (what the literature says, the gap, the design the experiment will run) after the literature and before the design. Set `pauses.plan: ask`, or answer *Stop to read and edit the plan* in the `@fi /new` interview (advanced), and the quest stops once it is written. `@fi /plan <quest_id>` opens the file beside the chat: edit it (the design block at the end is used exactly as written), or ask for a change with `@fi /plan <quest_id> <what to change>` and the plan is rewritten and reopened; repeat, then `@fi /resume <quest_id>` runs it. The plan's design block also holds an experiment protocol (the grid, the runs per setting, the thresholds): a script that sweeps other values than the plan fixed is sent back twice, and if it still differs the quest stops with an *Action needed* message naming what differs; fix the script or the plan and `@fi /resume <quest_id>`. The protocol also declares oracles (checks that do not rely on the script's own numbers): the script is run once with `FI_ORACLE=1` to measure them before its main run (the engine judges each value against the `expected` and `tolerance` the protocol fixes; the script's own pass/fail is not read), and a quest whose script does not pass stops with an *Action needed* message naming what failed. A run whose own numerics warned (an overflow, a solver that did not converge, an argument that had no effect) is sent back like a failed one and stops the quest the same way if the repairs do not remove the warnings; `@fi /resume <quest_id>` runs the script again if you changed it, and accepts the run if you did not. A simulation whose `run_manifest.json` (what it really swept and ran) differs from the frozen protocol is sent back once and then stops the quest the same way (*Action needed*: fix `code/simulate.py` and `@fi /resume <quest_id>`); with `execution.split_failure: block` a code-writing reply that does not hold both scripts stops it too. A finished quest states how far its result was checked: `[FI] evidence: <level>` (`executed`, `internally_consistent`, `validated_against_oracle` or `publication_ready`), followed by what stands between it and the next level; the same record is in the quest's `needs/EVIDENCE.json`.
