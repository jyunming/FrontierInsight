...
5. **Multi-model ensemble** — off (a single model, the default) or one of three fan-out profiles that run selected nodes across three models and merge the answers: `cross_check` alone, then `ideate` as well, then `analyze` too. Each option states its cost multiplier (~1.3× / ~2.0× / ~2.5×), since that is the trade-off being made. Maps to `provider.node_ensemble`.
...
- `@fi /approve-all-skills` — approve every skill that passes its gates in one go, after optionally pip-installing what the quarantined ones are missing (a skill whose library is absent fails its self-test and cannot be approved, so the order is install → re-test → approve). Still asks who is approving, still refuses a failing self-test, and still holds high-severity findings unless you confirm them.
