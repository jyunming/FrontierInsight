...
5. **Multi-model ensemble** — off (a single model, the default) or one of three fan-out profiles that run the same node across three models and merge the answers. Each option states its cost multiplier (~1.3× / ~2.0× / ~2.5×), since that is the trade-off being made. Maps to `provider.node_ensemble`.
