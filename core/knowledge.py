...
async def _route_sources_with_llm(
    ...
    try:
        ...
        prompt = (
            ...
            "MATCH SOURCES TO THE TOPIC'S FIELD: arXiv is physics / CS / math "
            "preprints ONLY and PubMed is biomedical ONLY — do NOT pick them for "
            "humanities, arts, history, culture, business, or current-events "
            "topics. For those prefer the multidisciplinary indices (OpenAlex, "
            "Crossref); the always-on web search already covers popular / "
            "encyclopedic / museum / trade sources.\n\n"
            "# Topic\n"
            f"{topic[:1500]}\n"
            f"{idea_text}\n"
            "# Source catalog\n"
