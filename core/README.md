...
| `provider.py` | Unified async LLM client. One `LLMClient.chat(messages) -> str` surface over four transports (`ResolvedEndpoint.transport` ∈ `"http"` / `"cli"` / `"vscode_bridge"`, with the proxy providers also routing via `"http"` to a locally-spawned proxy): HTTP-direct (openai/codex/gemini/ollama/vllm), HTTP-via-proxy (`claude_code` + the deprecated `github_copilot_*`), CLI-exec (`claude_cli` / `codex_cli` / `copilot_cli` / `gemini_cli`), and the VSCode-extension bridge (`vscode_extension`, paired with `vscode_bridge.py`). Includes `ProxySupervisor` for ref-counted proxy spawn. |
