...
class ExternalSkillDirs:
    ...
    A caller with no quest config of its own (``--skills`` / ``--why-skills`` /
    ``--scan-skill`` / ``--approve-skill`` / ``--revoke-skill`` given no
    ``--config``, and the web skills page and the VSCode chat panel given no config
    path) passes none, and gets the default: no folder named, the usual places
    searched. Given ``--config quest.yaml`` (or that path in the page's Quest config
    field, or after the chat command) they build this from that quest's own
